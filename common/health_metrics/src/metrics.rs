...
pub fn scrape_health_metrics_with_datadir(datadir: Option<&std::path::Path>) {
    scrape_process_health_metrics();
    scrape_system_health_metrics(datadir);
}
...
pub fn scrape_system_health_metrics(datadir: Option<&std::path::Path>) {
    // This will silently fail if we are unable to observe the health. This is desired behaviour
    // since we don't support `Health` for all platforms.
    let health_result = match datadir {
        Some(_) => {
            #[cfg(target_os = "linux")]
            {
                SystemHealth::observe_with_datadir(datadir)
            }
            #[cfg(not(target_os = "linux"))]
            {
                SystemHealth::observe()
            }
        }
